...
class CptnRuby < (Example rescue Gosu::Window)
  ...
  def update
    ...
    # move_x -= 5 if (Gosu::button_down? Gosu::KbLeft or Gosu::button_down? Gosu::Gp0Left)
    if [Gosu::KbRight, Gosu::GpButton5, Gosu::GpButton14].find(&Gosu.method(:button_down?))
      move_x += 5
    end
    # move_x += 5 if (Gosu::button_down? Gosu::KbRight or Gosu::button_down? Gosu::Gp0Right)
    @cptn.update(move_x)
  ...
  def button_down(id)
    if (id == Gosu::KbUp) or (id== Gosu::GpButton0 ) then
      @cptn.try_to_jump
      @beep.play
    end
  end
